...
const Index = () => {
    ...
    const downloadExcel = async () => {
        ...
        await postRouteAPI(body).then((res) => {
            ...
            const newPurchaseOrderList = res.data.data.map((item) => {
                return {
                    ...item,
                    key: item.id
                }
            ...
            if (!newPurchaseOrderList || newPurchaseOrderList.length === 0) {
            ...
            // Build each data row from the newPurchaseOrderList based on the tableColumns keys
            const rows = newPurchaseOrderList.map(item => {
            ...
            console.log(err);
            toast.error("حدث خطأ أثناء إعداد التقرير, يرجى المحاولة مرة أخرى.", { rtl: true, });
            setIsLoading(false);
        })
    };
