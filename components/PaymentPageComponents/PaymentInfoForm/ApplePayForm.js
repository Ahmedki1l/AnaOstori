...
export default function ApplePayForm(props) {
	const { checkoutID, orderID } = props;

	useEffect(() => {
		// Set wpwlOptions before loading the HyperPay script
		window.wpwlOptions = {
			applePay: {
				displayName: "AnaOstori",
				total: { label: "Sanam Company, INC." },
		...
		// Load the HyperPay script dynamically
		const applePayCardForm = document.createElement('script');
		applePayCardForm.src = `${process.env.NEXT_PUBLIC_HYPERPAY}/v1/paymentWidgets.js?checkoutId=${checkoutID}`;
		applePayCardForm.async = true;

		applePayCardForm.onerror = () => {
			console.error('Failed to load paymentWidgets.js script');
		};
		...
		// Cleanup
		return () => {
			if (applePayCardForm) document.head.removeChild(applePayCardForm);
		};
	}, [checkoutID]);
	...
	return (
		<div>
			<form
				action={`${process.env.NEXT_PUBLIC_WEB_URL}/payment?orderId=${orderID}`}
