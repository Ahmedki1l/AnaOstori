...
const SimulationExamComponent = ({
    ...
    const handleCopyLink = useCallback((examId) => {
        // adjust this to whatever your public exam URL pattern is
        const url = `https://www.anaostori.com/myCourse/exam?examId=${examId}`;
        navigator.clipboard.writeText(url)
            .then(() => toast.success('تم نسخ رابط الامتحان!'))
                            ...
                            <tbody className={styles.tableBodyArea}>
                                {questionsData.map((item) => (
                                    <tr className={styles.tableRow} key={item.id}>
                                        ...
                                        <td>{typeOfListdata === "folder" ? fullDate(item?.createdAt) : item?.type || "متعدد الخيارات"}</td>
                                            ...
                                            <div className={styles.eventButtons}>
                                                ...
                                                {typeOfListdata === 'exam' && (
                                                    <div onClick={() => handleCopyLink(item._id)}>
                                                        <AllIconsComponenet iconName='copy' height={24} width={24} color={'#000000'} />
                                                    </div>
                                                )}
