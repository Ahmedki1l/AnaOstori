...
const Appointments = ({
  ...
  const [allAppointmentList, setAllAppointmentList] =
    useState(availabilityList);
  ...
  const [isAppointmentPublished, setIAppointmentPublished] = useState(
    editAvailability ? editAvailability.published : true
  );
  const [isContentAccess, setIsContentAccess] = useState(
    editAvailability ? editAvailability.contentAccess : false
  );
  ...
  const getRegionLIst = async () => {
    await getRouteAPI({ routeName: "listRegion" })
      .then((res) => {
  ...
  const calculateNumberOfSeats = (newMaxSeats) => {
    return (
      editAvailability?.numberOfSeats +
  ...
  const onFinish = async (values) => {
    ...
    values.dateTo = dayjs(values?.dateTo?.$d)
      .endOf("day")
      .format("YYYY-MM-DD HH:mm:ss");
    ...
    values.numberOfSeats = isAvailabilityEdit
      ...
      : values.maxNumberOfSeats;
    if (!values.gender) values.gender = "mix";
    if (courseType == "online") delete values.name;
    if (isAvailabilityEdit) {
      if (
        values.maxNumberOfSeats <
        editAvailability.maxNumberOfSeats - editAvailability?.numberOfSeats
      ) {
        toast.error(
          "max number of seates are not less then number of enrolled students",
          { rtl: true }
        );
        setShowBtnLoader(false);
        return;
      ...
      await postRouteAPI(body)
        .then((res) => {
          availabilitySuccessRes(
            toastSuccessMessage.appoitmentUpdateSuccessMsg
          );
        })
        .catch(async (error) => {
          console.log(error);
      ...
      await postRouteAPI(values)
        .then((res) => {
          availabilitySuccessRes(
            toastSuccessMessage.appoitmentCretedSuccessMsg
          );
        })
        .catch(async (error) => {
          if (error?.response?.status == 401) {
  ...
  const handlePublishedAppointment = async (e, availabilityId) => {
    ...
    await postRouteAPI(body)
      .then((res) => {
        ...
        availabilitySuccessRes(
          e
            ? toastSuccessMessage.appointmentSuccessShowMsg
            : toastSuccessMessage.appointmentSuccessHiddenMsg
        );
        getAllAvailability();
      })
      .catch(async (error) => {
  ...
  const selectRegionFilter = (value) => {
    if (value == "all") {
      ...
      setAllAppointmentList(
        storeData?.availabilityList.filter((obj) => obj.regionId == value)
      );
    }
  };
      ...
      <div>
        ...
        <table className={styles.tableArea}>
          <thead className={styles.tableHeaderArea}>
            <tr>
              <th className={`${styles.tableHeadText} ${styles.tableHead1}`}>
                بيانات الفترة
              </th>
              ...
              <th className={`${styles.tableHeadText} ${styles.tableHead3}`}>
                تاريخ الإنشاء
              </th>
              <th className={`${styles.tableHeadText} ${styles.tableHead4}`}>
                اخر تعديل
              </th>
              <th className={`${styles.tableHeadText} ${styles.tableHead5}`}>
                المسجلين
              </th>
              <th className={`${styles.tableHeadText} ${styles.tableHead6}`}>
                الإجراءات
              </th>
            </tr>
          </thead>
                  ...
                  <tr key={`tableRow${index}`} className={styles.tableRow}>
                      ...
                      <div className={styles.PeriodDataDetails}>
                        ...
                        <div className={styles.genderDetails}>
                          {appointment.numberOfSeats > 5 ? (
                            <div
                              className={`${styles.outerCircle} ${styles.green}`}
                            >
                    ...
                    <td>
                      <div className={styles.publishState}>
                        <Switch
                          defaultChecked={appointment.published}
                          onChange={handlePublishedAppointment}
                          params={appointment.id}
                        ></Switch>
                        <p className="pr-2">
                          {appointment.published ? "إظهار" : "مخفي"}
                        </p>
                      </div>
                    </td>
                    <td className="py-2">
                      {appointment.instructors.map((instructor, index) => {
                        return (
                          <div
                            key={`instructor${index}`}
                            className={styles.instructorDetailsArea}
                          >
                            <div>
                              <ProfilePicture
                                height={34}
                    ...
                    <td>
                      <div
                        className={styles.personeDetails}
                        onClick={() => handleStudentData(appointment)}
                      >
                        <AllIconsComponenet
                          iconName={"personegroup"}
                          height={18}
                          width={24}
                          color={"#F26722"}
                          backColor={"#000000"}
                        />
                        <p className={styles.studentCountWrapper}>
                          {appointment.maxNumberOfSeats -
                            appointment.numberOfSeats}{" "}
                          طالب
                        </p>
                      </div>
                    </td>
          ...
          >
            <div className={styles.modalHeader}>
              ...
              <p className={`fontBold ${styles.createappointment}`}>
                {editAvailability ? "تعديل بيانات الموعد" : "إضافة موعد"}
              </p>
            </div>
            <Form form={appointmentForm} onFinish={onFinish}>
              <div className={styles.createAppointmentFields}>
                {courseType == "physical" && (
                  <>
                    ...
                    <FormItem
                      name={"name"}
                      rules={[
                        { required: false, message: "لازم تكتب العنوان" },
                      ]}
                    >
                      <Input
                        fontSize={16}
                ...
                {courseType === "online" && (
                  <>
                    <p className={`${styles.createappointmentFormFileds}`}>
                      كود Google classroom
                    </p>
                    <FormItem name={"classRoomCode"}>
                      <Input
                        fontSize={16}
                        width={352}
                        height={40}
                ...
                {showSwitchBtn && (
                  <>
                    <div className="flex items-center">
                      <Switch
                        defaultChecked={editAvailability.contentAccess}
                        onChange={onChangeContentAccess}
                      ></Switch>
                      <p className={styles.recordedcourse}>
                        إظهار المقرر للطلاب
                      </p>
                    </div>
                    {/* <div className='flex items-center'>
